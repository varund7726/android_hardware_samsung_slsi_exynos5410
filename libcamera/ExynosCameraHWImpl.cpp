          ...
          m_halDevice(dev)
{
    ...
    m_pictureRunning2 = false;
    m_sensorRunning = false;
    m_sensorRunning2 = false;
#ifdef OTF_SENSOR_LPZSL
    ...
    m_previewBuf2.virt.extP[0] = NULL;
...
*/
    ...
    if (m_captureMode) {
        ...
        if (m_previewRunning == true) {
            ...
            if (m_secCamera->flagStartIsp2() == true &&
                m_secCamera->stopIsp2() == false)
                CLOGE("ERR(%s):on m_secCamera->stopIsp2() fail", __func__);
            ...
            if (m_secCamera->flagStartPreview2() == true &&
                m_secCamera->stopPreview2() == false)
                CLOGE("ERR(%s):Fail on m_secCamera->stopPreview()", __func__);
            ...
            if (m_pictureRunning2 == true &&
                m_stopPictureInternal2() == false)
                CLOGE("ERR(%s):m_stopPictureInternal2() fail", __func__);
...
#endif
                if (m_pictureRunning2 == true) {
                    if (m_stopPictureInternal2() == false)
                        CLOGE("ERR(%s):m_stopPictureInternal2() fail", __func__);

                    if (m_startPictureInternal2() == false)
                        CLOGE("ERR(%s):m_startPictureInternal2() fail", __func__);
                }
            }
...
void ExynosCameraHWImpl::release()
{
    ...
    if (m_ispThread2 != NULL) {
        m_ispLock2.lock();
        m_ispThread2->requestExit();
        m_exitIspThread2 = true;
        m_ispCondition2.signal();
        m_ispLock2.unlock();

        m_ispThread2->requestExitAndWait();
        m_ispThread2.clear();
    }
    ...
    if (m_sensorThread2 != NULL) {
        m_sensorThread2->requestExit();
        m_exitSensorThread2 = true;
        m_sensorRunning2 = true;
        m_sensorCondition2.signal();
        m_sensorThread2->requestExitAndWait();
        m_sensorThread2.clear();
    }
    ...
    for (int i = 0; i < NUM_OF_PICTURE_BUF; i++) {
        if (m_pictureHeap2[i]) {
            m_pictureHeap2[i]->release(m_pictureHeap2[i]);
            m_pictureHeap2[i] = 0;
            m_pictureHeap2Fd[i] = -1;
        }
    }
...
status_t ExynosCameraHWImpl::m_startSensor2()
{
    CLOGD("[%s] (%d)", __func__, __LINE__);

    Mutex::Autolock lock(m_sensorLock2);
...
void ExynosCameraHWImpl::m_stopSensor2()
{
    CLOGD("[%s] (%d)", __func__, __LINE__);

    if (m_sensorRunning2 == true) {
        m_sensorRunning2 = false;

        m_sensorStopLock2.lock();
        m_sensorCondition2.signal();

        /* wait until sensor thread is stopped */
        m_sensorStoppedCondition2.wait(m_sensorStopLock2);
        m_sensorStopLock2.unlock();
    } else {
        CLOGV("DEBUG(%s):sensor2 not running, doing nothing", __func__);
    }
}

bool ExynosCameraHWImpl::m_sensorThreadFuncWrap2(void)
{
    while (1) {
        m_sensorStopLock2.lock();

        while (m_sensorRunning2 == false) {

            if (m_secCamera->flagStartSensor2() == true) {
                if (m_secCamera->stopSensor2() == false)
                    CLOGE("ERR(%s):Fail on m_secCamera->stopSensor2()", __func__);
            }
            CLOGV("DEBUG(%s):waiting", __func__);

            m_sensorStoppedCondition2.signal();
            m_sensorCondition2.wait(m_sensorStopLock2);
            CLOGV("DEBUG(%s):return from wait", __func__);
        }
        m_sensorStopLock2.unlock();

        if (m_exitSensorThread2 == true) {
            m_sensorStopLock2.lock();
            if (m_secCamera->flagStartSensor2() == true &&
                m_secCamera->stopSensor2() == false)
                CLOGE("ERR(%s):Fail on m_secCamera->stopSensor2()", __func__);
...
bool ExynosCameraHWImpl::m_sensorThreadFunc2(void)
{
    ExynosBuffer sensorBuf2;
    ExynosBuffer ispBuf2;

    m_sensorLock2.lock();

    if (m_secCamera->getSensorBuf2(&sensorBuf2) == false) {
        CLOGE("ERR(%s):getSensorBuf2() fail", __func__);
        m_sensorLock2.unlock();
        return false;
    }
    m_sensorLock2.unlock();

    if (isp_check > 4 || isp_input_count2 > 0) {
        CLOGW("D_1 %d %d", isp_check, isp_input_count2);
        goto done;
    }

    ExynosCameraHWImpl::isCh0Mutex.lock();
    if (m_secCamera->getIs3a0Buf(SENSOR_FRONT, &sensorBuf2, &ispBuf2) == false) {
        CLOGE("ERR(%s):getIs3a0Buf() fail(id:%d)", __func__, getCameraId());
    ...
    if (ispBuf2.reserved.p < 0) {
        CLOGW("WRN(%s): ispBuf.reserved.p = %d", __func__, ispBuf2.reserved.p);
        goto done;
    }

    m_sharedISPBuffer2 = ispBuf2;

    isp_check++;
    isp_input_count2++;
    if (m_secCamera->putISPBuf2(&ispBuf2) == false) {
        CLOGE("ERR(%s):putISPBuf() fail", __func__);
        return false;
    }
    m_ispCondition2.signal();
    ...
    m_sensorLock2.lock();
    if (m_secCamera->putSensorBuf2(&sensorBuf2) == false) {
        CLOGE("ERR(%s):putSensorBuf2() fail", __func__);
        m_sensorLock2.unlock();
...
bool ExynosCameraHWImpl::m_ispThreadFunc2(void)
{
    ExynosBuffer ispBuf;

    m_ispLock2.lock();
    if (m_exitIspThread2 == true) {
        m_ispLock2.unlock();
        CLOGD("DEBUG(%s):exit 0", __func__);
        return true;
    }

    m_ispCondition2.wait(m_ispLock2);

    if (m_exitIspThread2 == true || m_previewRunning == false) {
        m_ispLock2.unlock();
        CLOGD("DEBUG(%s):exit 1", __func__);
        return true;
    }
    ...
    if (isp_input_count2 > 1)
        CLOGV("[%s] (%d)", __func__, __LINE__);

    for (int i = 0; i < isp_input_count2; i++) {
        if (m_secCamera->getISPBuf2(&ispBuf) == false) {
            CLOGE("ERR(%s):getISPBuf() fail", __func__);
...
bool ExynosCameraHWImpl::m_startPictureInternal2(void)
{
    CLOGD("[%s] (%d)", __func__, __LINE__);

    if (m_pictureRunning2 == true) {
        CLOGE("ERR(%s):Aready m_pictureRunning2 is running", __func__);
        return false;
    }
    ...
    pictureFormat = m_secCamera->getPictureFormat();
    m_secCamera->getPictureSize2(&pictureW, &pictureH);
    ...
    m_rawHeap = m_getMemoryCb(-1, pictureFramesize, 1, &m_rawHeapFd);
    if (!m_rawHeap || m_rawHeapFd <= 0) {
        CLOGE("ERR(%s):m_getMemoryCb(m_rawHeap, size(%d) fail", __func__, pictureFramesize);
        return false;
    }
    m_rawHeapSize = pictureFramesize;
    ...
    for (int i = 0; i < NUM_OF_PICTURE_BUF; i++) {
        ...
        m_pictureHeap2[i] = m_getMemoryCb(-1, pictureFramesize, 1, &(m_pictureHeap2Fd[i]));
        if (!m_pictureHeap2[i] || m_pictureHeap2[i] <= 0) {
            CLOGE("ERR(%s):m_getMemoryCb(m_pictureHeap2[%d], size(%d) fail", __func__, i, pictureFramesize);
            return false;
        }

        m_pictureBuf2 = nullBuf;
        m_pictureBuf2.virt.extP[0] = (char *)m_pictureHeap2[i]->data;
        m_pictureBuf2.fd.extFd[0] = m_pictureHeap2Fd[i];
        m_getAlignedYUVSize(pictureFormat, ALIGN_UP(pictureW, CAMERA_MAGIC_ALIGN), ALIGN_UP(pictureH, CAMERA_MAGIC_ALIGN), &m_pictureBuf2);
        m_pictureBuf2.reserved.p = i;

        m_secCamera->setPictureBuf2(&m_pictureBuf2);
    }

    if (m_secCamera->startPicture2() == false) {
        CLOGE("ERR(%s):Fail on m_secCamera->startPicture2()", __func__);
        return false;
    }

    m_pictureBuf2 = nullBuf;

    m_pictureRunning2 = true;

    return true;
}

bool ExynosCameraHWImpl::m_stopPictureInternal2(void)
{
    CLOGD("[%s] (%d)", __func__, __LINE__);

    if (m_pictureRunning2 == false) {
        CLOGE("ERR(%s):Aready m_pictureRunning2 is stop", __func__);
        return false;
    }

    if (m_secCamera->flagStartPicture2() == true
        && m_secCamera->stopPicture2() == false)
        CLOGE("ERR(%s):Fail on m_secCamera->stopPicture2()", __func__);
